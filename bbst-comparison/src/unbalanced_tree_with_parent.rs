...
impl crate::test_utils::HasRoot for UnbalancedTreeWithParent {
    type Node = Node;
...
mod test {
    ...
    fn generate_benchmark_queries() -> (UnbalancedTreeWithParent, Vec<Query>) {
        let mut rng = StdRng::seed_from_u64(42);
        ...
        let initial_values: Vec<i32> = (0..n_initial)
            ...
            .collect();
        let tree: UnbalancedTreeWithParent = initial_values.into_iter().collect();
    ...
    fn analyze_tree_stats() {
        ...
        for (i, &query) in queries.iter().enumerate() {
            ...
            if (i + 1) % 20_000 == 0 {
                ...
                println!(
                    "After {:6} queries: len={:6}, height={:2} (optimal: ~{:.1}, ratio: {:.2})",
                    i + 1,
                    len,
                    height,
                    optimal_height,
